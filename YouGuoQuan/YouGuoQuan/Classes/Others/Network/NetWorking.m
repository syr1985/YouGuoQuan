...
+ (void)getSessionWithUrl:(NSString *)urlStr parameters:(id)parameters success:(void (^)(id))success fail:(void (^)(NSError *, NSString *))fail progress:(void (^)(float))progress {
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithArray:@[@"image/jpeg",@"application/json"]];
    
    [manager GET:urlStr parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        if (progress) {
            float complete = (float)downloadProgress.completedUnitCount / downloadProgress.totalUnitCount;
            progress(complete);
        }
