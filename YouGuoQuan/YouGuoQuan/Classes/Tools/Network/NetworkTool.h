 ...
 *  商品的评论列表
 */
static NSString * const Detail_GetGoodsComments    = @"/iface/goodscomment/list";
/**
 *  动态详情
 */
 ...
 *  支付-支付成功后检查订单状态是否改变
 */
static NSString * const Pay_CheckOrderNo_Nobility  = @"/iface/payResult/payOk";//@"/iface/goods/checkNotice";
/**
 *  动态-删除
 */
 ...
 *  我的-订单-申请退款
 */
static NSString * const Profile_Order_ApplyRefund  = @"/iface/goodsOrder/applyReturnMoney";
/**
 *  我的-售出订单
 */
 ...
 *  发现-获取我的众筹金额
 */
static NSString * const Discovery_GetMyContriMoney = @"/iface/crowdfunding/myContriMoney";
/**
 *  我的-钱包-提现
 */
static NSString * const Profile_WalletWithdrawCash = @"/iface/withdrawalsrequest/create";
/**
 *  消息-清空系统消息
 */
static NSString * const Message_ClearSystemMessage = @"/iface/appsysmessage/delMyMessage";
/**
 *  我的-卖家同意退款
 */
static NSString * const Profile_SellerAgreeRefund  = @"/iface/goods/sellerAgreeRefund";
/**
 *  我的-出售微信-买家列表
 */
static NSString * const Profile_SellWX_OrderList   = @"/iface/goodsOrder/wxlist";
/**
 *  我的-订单-刷新支付订单号（商品订单号与支付订单号分开）
 */
static NSString * const Profile_RefreshPayOrderNo  = @"/iface/goods/refreshPayOrderNo";
/**
 *  我的-删除照片墙图片
 */
static NSString * const Profile_DelPhotoWallImage  = @"/iface/appuserphoto/del";
/**
 *  我的-钱包充值-验证
 */
static NSString * const Profile_IAP_VerifyReceipt  = @"/iface/payResult/setIapCertificate";
/**
 *  我的-订单-取消订单
 */
static NSString * const Profile_Order_CancelOrder  = @"/iface/goodsOrder/cancelOrder";
/**
 *  我的-设置-推广
 */
static NSString * const Profile_MyPromotionIncome  = @"/iface/payvoucher/promotionIncome";
//******------------------------------------------------------------------******//
...
/**
 *  继续支付里根据原来的订单号刷新支付订单号
 *
 *  @param orderNo 订单号
 *  @param success 成功block
 *  @param failure 失败block
 */
+ (void)refreshPayOrderNoWithOrderNo:(NSString *)orderNo success:(void (^)(id result))success failure:(void (^)())failure;
/**
 *  我的-出售微信-买家订单列表
 *
 *  @param dealType 1，已处理；2，未处理；参数为空代表全部订单
 *  @param pageNo   页码
 *  @param pageSize 每页数
 *  @param success  成功block
 *  @param failure  失败block
 ...
 *
 *  @param orderNo 订单号
 *  @param salerOrBuyer 卖家或买家 B or S
 *  @param success 成功block
 *  @param failure 失败block
 */
 ...
 *  @param zfbAccount  身份证号
 *  @param authentiUrl 认证图片地址
 *  @param realName    真实姓名
 *  @param mobile      手机号
 *  @param auditType   认证类型 2申请个人认证 4申请团队认证
 *  @param success     成功block
 *  @param failure     失败block
 */
 ...
 *  单项修改我的资料
 *
 *  @param mType    修改属性 mtype取值headImg nickName sex city height weight constellatory work emotion coverImgUrl
 *  @param newValue 新值
 *  @param success  成功block
 *  @param failure  失败block
 */
 ...
 *  @param failure 失败block
 */
+ (void)checkOrderStatusOrderNo:(NSString *)orderNo success:(void (^)())success failure:(void(^)(NSError *error))failure;
/**
 *  支付贵族
 *
 *  @param orderNo 订单号
...
//+ (void)uploadVideoALAssetToQiniu:(ALAsset *)asset progress:(QNUpProgressHandler)progress success:(void(^)(NSString *videoUrl))success failure:(void(^)())failure;
/**
 *  图片或视频鉴黄
 *
 *  @param url     图片链接
